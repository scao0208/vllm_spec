#!/bin/bash
# nsys profile: SGLang STANDALONE chain + FLy — NVTX markers for draft/verify
...
# Use fewer prompts for profiling (nsys trace files get large)
NUM_PROMPTS=20

nsys profile \
    -t cuda,nvtx,osrt \
    --cuda-memory-usage=true \
    -o nsys_fly_chain_nvtx \
    ...
    --fly-window-size 6 \
    --profile-nvtx \
    2>&1 | tee nsys_fly_chain_nvtx.log
