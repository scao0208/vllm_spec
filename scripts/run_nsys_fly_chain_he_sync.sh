#!/bin/bash
# nsys profile: SGLang STANDALONE chain + FLy — NVTX markers WITH cuda.synchronize()
# This gives accurate GPU-inclusive timing per phase (draft vs verify)
# CUDA graphs ENABLED (no --disable-cuda-graph)
...
# Use fewer prompts for profiling (--cuda-graph-trace=node generates much more data)
NUM_PROMPTS=10
...
nsys profile \
    ...
    --fly-window-size 6 \
    --profile-nvtx-sync \
    2>&1 | tee nsys_fly_chain_nvtx_sync.log
